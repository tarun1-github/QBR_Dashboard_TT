...
# Keep application imports rooted at the project package. Run Streamlit from
# the repository root: streamlit run app/dashboard.py
...
def msg(kind, title, detail=""):
    ...
    st.markdown(
        f'<div class="qbr-msg {cls}">{icon}&nbsp;&nbsp;<b>{title}</b> {detail}</div>',
        unsafe_allow_html=True,
    )
...
def role():
    return str(user().get("RoleName") or user().get("role") or "").upper()


def uname():
    return str(user().get("Username") or user().get("username") or "")


def dname():
    return str(user().get("DisplayName") or user().get("name") or uname())
...
def assigned_tracks(username):
    ...
    try:
        if not db.execute(
            text("SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='qbr' AND TABLE_NAME='UserTrackAccess'")
        ).first():
            return []
        if db.execute(
            text("SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='qbr' AND TABLE_NAME='UserTrackAccess' AND COLUMN_NAME='TowerTrackID'")
        ).first():
            q = """SELECT tt.TowerName,tt.TrackName
                   FROM qbr.UserTrackAccess a
                   JOIN qbr.AppUser u ON u.UserID=a.UserID
                   JOIN qbr.TowerTrack tt ON tt.TowerTrackID=a.TowerTrackID
                   WHERE LOWER(u.Username)=LOWER(:u) AND ISNULL(tt.IsActive,1)=1
                   ORDER BY tt.TowerName,tt.TrackName"""
        else:
            q = """SELECT t.TowerName,tr.TrackName
                   FROM qbr.UserTrackAccess a
                   JOIN qbr.AppUser u ON u.UserID=a.UserID
                   JOIN qbr.Track tr ON tr.TrackID=a.TrackID
                   JOIN qbr.Tower t ON t.TowerID=tr.TowerID
                   WHERE LOWER(u.Username)=LOWER(:u) AND ISNULL(tr.IsActive,1)=1
...
def admin_rows():
    db = SessionLocal()
    try:
        q = """SELECT u.DisplayName,u.Username,u.RoleName,tt.TowerName,tt.TrackName,
                       uta.CanView,uta.CanExport,uta.CanManage,uta.UserTrackAccessID
                FROM qbr.UserTrackAccess uta
                JOIN qbr.AppUser u ON u.UserID=uta.UserID
                JOIN qbr.TowerTrack tt ON tt.TowerTrackID=uta.TowerTrackID
                WHERE ISNULL(tt.IsActive,1)=1
                ORDER BY tt.TowerName,tt.TrackName,u.DisplayName"""
...
def manager_users():
    ...
    try:
        return db.execute(
            text("SELECT UserID,Username,DisplayName,RoleName,IsActive FROM qbr.AppUser WHERE IsActive=1 ORDER BY DisplayName")
        ).mappings().all()
    finally:
        db.close()
...
def assign_manager(username, tower, track):
    ...
    try:
        u = db.execute(
            text("SELECT UserID,DisplayName,RoleName FROM qbr.AppUser WHERE LOWER(Username)=LOWER(:u)"),
            {"u": username},
        ).mappings().first()
        tt = db.execute(
            text("SELECT TowerTrackID FROM qbr.TowerTrack WHERE TowerName=:t AND TrackName=:tr AND IsActive=1"),
            {"t": tower, "tr": track},
        ).mappings().first()
        if not u or not tt:
            return False, "User or track not found."
        if str(u["RoleName"]).upper() != "MANAGER":
            return False, f'{u["DisplayName"]} current role is {u["RoleName"]}. Only MANAGER can be assigned.'
        exists = db.execute(
            text("""SELECT u.DisplayName,tt.TowerName,tt.TrackName
                    FROM qbr.UserTrackAccess a
                    JOIN qbr.AppUser u ON u.UserID=a.UserID
                    JOIN qbr.TowerTrack tt ON tt.TowerTrackID=a.TowerTrackID
                    WHERE a.UserID=:uid"""),
        ...
        if exists:
            return False, f'{u["DisplayName"]} is already assigned to {exists["TowerName"]} → {exists["TrackName"]}. One track per manager.'
        occupied = db.execute(
            text("""SELECT u.DisplayName FROM qbr.UserTrackAccess a
                    JOIN qbr.AppUser u ON u.UserID=a.UserID
                    WHERE a.TowerTrackID=:tt AND UPPER(u.RoleName)='MANAGER'"""),
            {"tt": tt["TowerTrackID"]},
        ).mappings().first()
        if occupied:
            return False, f'{tower} → {track} is already assigned to {occupied["DisplayName"]}. One manager per track.'
        db.execute(
            text("INSERT INTO qbr.UserTrackAccess(UserID,TowerTrackID,CanView,CanExport,CanManage) VALUES(:uid,:tt,1,1,0)"),
            {"uid": u["UserID"], "tt": tt["TowerTrackID"]},
        )
        db.commit()
        return True, f'{u["DisplayName"]} added to {tower} → {track} successfully.'
...
def remove_manager(username, tower, track):
    ...
    try:
        r = db.execute(
            text("""DELETE a FROM qbr.UserTrackAccess a
                    JOIN qbr.AppUser u ON u.UserID=a.UserID
                    JOIN qbr.TowerTrack tt ON tt.TowerTrackID=a.TowerTrackID
                    WHERE LOWER(u.Username)=LOWER(:u)
                      AND tt.TowerName=:t AND tt.TrackName=:tr"""),
            {"u": username, "t": tower, "tr": track},
        )
        if r.rowcount == 0:
            db.rollback()
            return False, f'{username} has no assignment on {tower} → {track}.'
        db.commit()
        return True, f'{username} removed from {tower} → {track} successfully.'
    except Exception as e:
        db.rollback()
        return False, str(e)
    finally:
...
def update_role(username, new_role):
    ...
    try:
        old = db.execute(
            text("SELECT DisplayName,RoleName FROM qbr.AppUser WHERE LOWER(Username)=LOWER(:u)"),
            {"u": username},
        ).mappings().first()
        if not old:
            return False, "User not found."
        db.execute(
            text("UPDATE qbr.AppUser SET RoleName=:r,UpdatedAt=SYSUTCDATETIME() WHERE LOWER(Username)=LOWER(:u)"),
            {"r": new_role, "u": username},
        )
        if new_role in ("SUPERUSER", "SUPERVISOR"):
            db.execute(
                text("DELETE a FROM qbr.UserTrackAccess a JOIN qbr.AppUser u ON u.UserID=a.UserID WHERE LOWER(u.Username)=LOWER(:u)"),
                {"u": username},
            )
        db.commit()
...
r = role()
title = "Supervisor Dashboard" if r == "SUPERVISOR" else "Manager Dashboard" if r == "MANAGER" else "Superuser Dashboard"
st.markdown(
    f'<div class="qbr-hero"><h1>📊 QBR Executive Dashboard</h1><p>HCLTech Customer Operations Command Center &nbsp;•&nbsp; <b>{title}</b> &nbsp;•&nbsp; Signed in: <b>{dname()}</b> &nbsp;•&nbsp; Role: <b>{r}</b></p></div>',
    unsafe_allow_html=True,
)

if st.session_state.get("show_change_password", False):
    st.markdown(
        '<div style="max-width:560px;margin:7vh auto 0;padding:30px 36px;border-radius:28px;background:linear-gradient(145deg,#fff,#eaf5f7);box-shadow:16px 16px 34px rgba(15,39,66,.18),-8px -8px 20px #fff;border:1px solid #d7e6ea;">',
        unsafe_allow_html=True,
    )
    ...
    current_pw = st.text_input("Current Password", type="password", key="cp_current", placeholder="Current password")
    new_pw = st.text_input("New Password", type="password", key="cp_new", placeholder="New password")
    confirm_pw = st.text_input("Confirm New Password", type="password", key="cp_confirm", placeholder="Confirm password")
    ...
    with cpa:
        if st.button("🔐 Update Password", use_container_width=True, key="cp_update"):
            ...
            if not valid:
                msg("bad", "Password policy failed.", "Use 8+ characters with uppercase, lowercase and a number.")
            elif new_pw != confirm_pw:
                msg("bad", "Passwords do not match.")
            else:
                db = SessionLocal()
                try:
                    if change_password(db, user()["UserID"], current_pw, new_pw):
                        ...
                        msg("bad", "Current password is incorrect.")
                except Exception as exc:
                    db.rollback()
                    msg("bad", "Unable to change password.", str(exc))
                finally:
                    db.close()
    with cpb:
...
with st.sidebar:
    ...
    if r == "MANAGER" and not allowed:
        st.markdown('<div class="qbr-msg inf">i&nbsp;&nbsp;<b>No track is assigned to this manager.</b> Ask a supervisor to assign one track.</div>', unsafe_allow_html=True)
    ...
    if tower == "All":
        tracks = sorted({d["TrackName"] for v in hierarchy.values() for d in v})
    else:
        tracks = [d["TrackName"] for d in hierarchy.get(tower, [])]
    if r == "MANAGER":
        tracks = [tr for tw, tr in allowed if tower == "All" or tw == tower]
    ...
    dr = st.date_input(
        "Report Date Range",
        value=(date(2026, 7, 15), min(date.today(), date(2026, 8, 31))),
...
if r == "MANAGER" and allowed:
    if tower == "All" and len(allowed) == 1:
        tower = allowed[0][0]
    if track == "All" and len(allowed) == 1:
        track = allowed[0][1]
...
# Normalize KPI values before rendering. This prevents Python's numeric format
# specifiers (e.g. :,.1f) from being applied to strings returned by SQL/DB code.
...
max_per_day = _safe_int(stats.get("max_count") if stats else 0)
min_per_day = _safe_int(stats.get("min_count") if stats else 0)
avg_per_day = _safe_float(stats.get("avg_count") if stats else 0)

st.markdown('<div class="qbr-section">📈 Executive KPI Snapshot</div>', unsafe_allow_html=True)
kpis = [
    ("🎫 TOTAL TICKETS", _safe_int(k.get("total")), "Selected timeframe", "linear-gradient(135deg,#0d5470,#1d8a9a,#23a58c)", "int"),
    ("👑 PARENT TICKETS", _safe_int(k.get("parents")), "Root workload", "linear-gradient(135deg,#4e7f32,#78a94d,#9bc96e)", "int"),
    ("↳ CHILD TICKETS", _safe_int(k.get("children")), "Linked workload", "linear-gradient(135deg,#e87525,#f19a45,#f6bd73)", "int"),
    ("⚡ ALERTS", alerts_total, "Monitoring events", "linear-gradient(135deg,#b80f17,#dc3038,#ef6868)", "int"),
    ("📊 MAX / MIN", f"{max_per_day:,} / {min_per_day:,}", "Tickets per day", "linear-gradient(135deg,#7d6008,#a7831b,#c4a340)", "text"),
    ("📈 AVG / DAY", avg_per_day, "Daily average", "linear-gradient(135deg,#315a9d,#4b7dcc,#789fe2)", "float"),
    ("🔵 OPEN", _safe_int(k.get("total")) - _safe_int(k.get("closed")), "Awaiting resolution", "linear-gradient(135deg,#166db1,#268bd0,#61b5eb)", "int"),
    ("✅ CLOSED", _safe_int(k.get("closed")), "Resolved", "linear-gradient(135deg,#2d8037,#52a85b,#82c886)", "int"),
    ("🔴 CRITICAL", _safe_int(k.get("critical")), "Critical priority", "linear-gradient(135deg,#b62222,#db4747,#ee7777)", "int"),
]
first_kpis = kpis[:5]
second_kpis = kpis[5:9]


def render_kpi(c, item):
    lab, val, sub, bg, value_type = item
    if value_type == "float":
        display_value = f"{float(val):,.1f}"
    elif value_type == "int":
        display_value = f"{int(val):,}"
    else:
        display_value = str(val)
    c.markdown(
        f'<div class="qbr-kpi" style="background:{bg}"><div class="t">{lab}</div><div class="v">{display_value}</div><div class="s">{sub}</div></div>',
        unsafe_allow_html=True,
    )


for c, item in zip(st.columns(5), first_kpis):
    render_kpi(c, item)
for c, item in zip(st.columns(4), second_kpis):
    render_kpi(c, item)

if view == "Day":
    trend = get_daily_trend(start, end, scope_tower, scope_track)
    x = "Date"
elif view == "Week":
    trend = get_weekly_trend(start, end, scope_tower, scope_track)
    x = "Week"
elif view == "Month":
    trend = get_monthly_trend(start, end, scope_tower, scope_track)
    x = "Month"
else:
    trend = get_quarterly_trend(start, end, scope_tower, scope_track)
    x = "Quarter"
...
with c1:
    ...
    if not trend.empty:
        ...
        for name, col, color in [("Parents", "Parents", "#19708b"), ("Children", "Children", "#ee8233"), ("Total", "Total", "#6b4f8f")]:
            if col in trend:
                fig.add_trace(go.Bar(x=trend[x], y=trend[col], name=name, marker=dict(color=color, line=dict(color="rgba(255,255,255,.8)", width=2)), text=trend[col], textposition="auto"))
        ...
        msg("inf", "No ticket trend data", "Try a wider date range.")
with c2:
    ...
    if not vol.empty:
        ...
        msg("inf", "No ticket volume data", "Check the selected Tower, Track and date range.")
...
with c1:
    ...
    if not pc_df.empty:
        ...
        st.plotly_chart(fig, use_container_width=True)
        with st.expander("🔍 View child ticket details"):
            st.dataframe(children_df, use_container_width=True, hide_index=True)
    else:
        msg("inf", "Parent-child data not available", "The dashboard found no child records for the selected scope.")
with c2:
    st.markdown("### ⚡ Highest Alert / Part Frequency")
    if not alerts.empty:
        d = alerts.head(12).sort_values("Count").copy()
        d["Label"] = d["Part"] + " • " + d["AlertType"]
        ...
        msg("inf", "Alert data not available", "Check the selected scope and date range.")
...
if not alert_summary.empty:
    ...
    msg("inf", "No alert summary data", "No alert rows match the selected filters.")
...
if r in ("SUPERUSER", "SUPERVISOR"):
    ...
    users = manager_users()
    active_managers = [u for u in users if str(u["RoleName"]).upper() == "MANAGER"]
    towers = list(hierarchy.keys())
    selected_tower = st.selectbox("Tower", towers, key="admin_tower")
    ...
    managers = st.selectbox(
        "Manager",
        [u["Username"] for u in active_managers],
        format_func=lambda x: next((u["DisplayName"] for u in active_managers if u["Username"] == x), x),
        key="admin_manager",
    ) if active_managers else None
    a, b, c = st.columns(3)
    with a:
        if st.button("➕ Assign Manager", use_container_width=True) and managers:
            ok, detail = assign_manager(managers, selected_tower, selected_track)
            msg("ok" if ok else "bad", "Assignment successful." if ok else "Assignment failed.", detail)
            if ok:
                st.rerun()
    with b:
        if st.button("➖ Remove Manager", use_container_width=True) and managers:
            ok, detail = remove_manager(managers, selected_tower, selected_track)
            msg("ok" if ok else "bad", "Removal successful." if ok else "Removal failed.", detail)
            if ok:
                st.rerun()
    with c:
    ...
    if rows:
        st.dataframe(
            pd.DataFrame(rows)[["DisplayName", "Username", "RoleName", "TowerName", "TrackName", "CanView", "CanExport", "CanManage"]].rename(
                columns={"DisplayName": "Manager", "RoleName": "Role", "TowerName": "Tower", "TrackName": "Track", "CanView": "View", "CanExport": "Export", "CanManage": "Manage"}
            ),
    ...
    role_user = st.selectbox(
        ...
        format_func=lambda x: next((f'{u["DisplayName"]} ({u["RoleName"]})' for u in users if u["Username"] == x), x),
        key="role_user",
    )
    ...
    if st.button("🛡️ Update Role", use_container_width=True):
        ok, detail = update_role(role_user, new_role)
        msg("ok" if ok else "bad", "Role updated successfully." if ok else "Role update failed.", detail)
        if ok:
            st.rerun()
...
summary = pd.DataFrame(
    [
        ...
        ["Parent Tickets", k["parents"]],
        ["Child Tickets", k["children"]],
        ["Alerts", alerts_total],
        ["Max Tickets/Day", max_per_day],
        ["Min Tickets/Day", min_per_day],
    ],
...
with pd.ExcelWriter(buf, engine="openpyxl") as writer:
    ...
    vol.to_excel(writer, index=False, sheet_name="Ticket Volume")
    alerts.to_excel(writer, index=False, sheet_name="Alert Frequency")
    pc_df.to_excel(writer, index=False, sheet_name="Parent Child")
    alert_summary.to_excel(writer, index=False, sheet_name="Alert Summary")
