...
# IMPORTANT: this makes `app` imports work when Streamlit executes
# `app/dashboard.py` directly from the repository root.
import sys
...
# Normalize DB values before formatting/rendering.
...
def render_kpi(column,item):
    lab,val,sub,bg,value_type=item
    if value_type=="float": display=f"{_safe_float(val):,.1f}"
    elif value_type=="int": display=f"{_safe_int(val):,}"
    else: display=str(val)
    column.markdown(f'<div class="qbr-kpi" style="background:{bg}"><div class="t">{lab}</div><div class="v">{display}</div><div class="s">{sub}</div></div>',unsafe_allow_html=True)
...
kpis=[
    ...
    ("⚡ ALERTS",_safe_int(alerts_total),"Monitoring events","linear-gradient(135deg,#b80f17,#dc3038,#ef6868)","int"),
    ...
    ("🔵 OPEN",max(0,_safe_int(k.get("total"))- _safe_int(k.get("closed"))),"Awaiting resolution","linear-gradient(135deg,#166db1,#268bd0,#61b5eb)","int"),
...
for c,item in zip(st.columns(5),first_kpis): render_kpi(c,item)
for c,item in zip(st.columns(4),second_kpis): render_kpi(c,item)
...
with c2:
    ...
    if not alerts.empty:
        d=alerts.groupby("Part",as_index=False)["Count"].sum().sort_values("Count").tail(10)
        fig=go.Figure(go.Bar(x=d.Count,y=d.Part,orientation="h",marker=dict(color=d.Count,colorscale=[[0,"#d6b24a"],[.5,"#ee8233"],[1,"#c91414"]],line=dict(color="white",width=2)),text=d.Count,textposition="auto",hovertemplate="<b>%{y}</b><br>Alerts: %{x}<extra></extra>"))
        fig.update_layout(height=400,paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="#f7fbfd",font=dict(family="Segoe UI",color="#12344d"),margin=dict(l=20,r=35,t=20,b=35),xaxis=dict(gridcolor="#dce8ed",title="Alert Count"),yaxis=dict(showgrid=False),showlegend=False)
        st.plotly_chart(fig,use_container_width=True)
...
if not alert_summary.empty:
    d=alert_summary.head(15).copy(); d["Label"]=d["Tower"]+" → "+d["Track"]; d=d.sort_values("TotalAlerts")
    fig=go.Figure(go.Bar(x=d.TotalAlerts,y=d.Label,orientation="h",marker=dict(color=d.TotalAlerts,colorscale=[[0,"#2c7e9a"],[.5,"#ee8233"],[1,"#c91414"]],line=dict(color="white",width=2)),text=d.TotalAlerts,textposition="auto",hovertemplate="<b>%{y}</b><br>Alerts: %{x}<extra></extra>"))
    fig.update_layout(height=470,paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="#f7fbfd",font=dict(family="Segoe UI",color="#12344d"),margin=dict(l=20,r=35,t=20,b=35),xaxis=dict(gridcolor="#dce8ed",title="Alerts"),yaxis=dict(showgrid=False),showlegend=False)
    st.plotly_chart(fig,use_container_width=True)
