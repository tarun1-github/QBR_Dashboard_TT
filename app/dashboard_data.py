...
def _normal_account_expr(alias: str) -> str:
    return (
        ...
        f"THEN 'Home Depot' ELSE LTRIM(RTRIM(ISNULL({alias}.CompanyAccount,''))) END"
    )


def _ticket_context(db, alias: str = "tk"):
    """Return SQL expressions that resolve every ticket to Customer/Tower/Track.

    Customer mapping is first and authoritative. Existing Ticket IDs are used
    as a second source so old rows remain reportable during migration.
    """
    ...
    if _table_exists(db, "Customer"):
        joins += (
            f" LEFT JOIN qbr.Customer cust ON "
            f"UPPER(LTRIM(RTRIM(ISNULL(cust.CompanyAccountName,''))))="
    ...
    track_parts = []
    if _table_exists(db, "Customer"):
        track_parts.append("NULLIF(LTRIM(RTRIM(tr_cust.TrackName)),'')") if False else None
        # Customer -> Track is joined below through TrackID.
        joins += (
            " LEFT JOIN qbr.Track tr_cust ON tr_cust.TrackID=cust.TrackID "
            "AND ISNULL(tr_cust.IsActive,1)=1"
        )
    ...
    # Assignment-group fallback is deliberately limited to known business names.
    ...
    if _table_exists(db, "Customer"):
        joins += (
            " LEFT JOIN qbr.Tower tw_cust ON tw_cust.TowerID=cust.TowerID "
            "AND ISNULL(tw_cust.IsActive,1)=1"
        )
    ...
    # Explicit business rule requested by QBR: all three Foundation tracks.
    tower_parts.append(
...
def get_tower_track_hierarchy():
    ...
    try:
        rows = db.execute(
            text(
                """
                SELECT t.TowerName,tr.TrackName
                FROM qbr.Track tr
                JOIN qbr.Tower t ON t.TowerID=tr.TowerID
                WHERE ISNULL(t.IsActive,1)=1 AND ISNULL(tr.IsActive,1)=1
                ORDER BY t.DisplayOrder,tr.DisplayOrder,tr.TrackName
                """
            )
        ).fetchall()
        result = {}
        for row in rows:
            tower = str(row.TowerName)
...
def get_executive_kpis(start_date=None, end_date=None, tower=None, track=None):
    ...
    try:
        ...
        monitoring = "CASE WHEN UPPER(LTRIM(RTRIM(ISNULL(tk.Caller,'')))) IN ('EMS','CMSP') THEN 1 ELSE 0 END" if "Caller" in cols else "0"
        q = f"""
            SELECT COUNT(*) Total,
                   SUM({parent}) Parents,
                   SUM({child}) Children,
        ...
        return {
            ...
            "parents": _safe_int(r.Parents),
            "children": _safe_int(r.Children),
            "closed": _safe_int(r.Closed),
            "monitoring": _safe_int(r.Monitoring),
            "critical": _safe_int(r.Critical),
            "high": _safe_int(r.High),
            "moderate": _safe_int(r.Moderate),
        }
    finally:
        db.close()
...
def get_alert_total(start_date=None, end_date=None, tower=None, track=None):
    """Compatibility name: monitoring-generated ticket count from qbr.Ticket."""
    db = SessionLocal()
    try:
        cols, joins, _tower, _track, scope = _ticket_context(db, "tk")
        ...
        date_col = "OpenedAt" if "OpenedAt" in cols else "CreatedAt"
        q = f"""
            SELECT COUNT(*) FROM qbr.Ticket tk {joins}
            WHERE UPPER(LTRIM(RTRIM(ISNULL(tk.Caller,'')))) IN ('EMS','CMSP')
              AND {_date_filter('tk',date_col)} AND {scope}
        """
...
def get_tower_track_volume(start_date=None, end_date=None, tower=None, track=None):
    ...
    try:
        ...
        date_col = "OpenedAt" if "OpenedAt" in cols else "CreatedAt"
        q = f"""
            SELECT b.Tower,b.Track,COUNT(*) Total,
                   SUM(CASE WHEN UPPER(ISNULL(b.TicketType,''))='PARENT' THEN 1 ELSE 0 END) Parents,
                   SUM(CASE WHEN UPPER(ISNULL(b.TicketType,''))='CHILD' THEN 1 ELSE 0 END) Children
            FROM (
...
def _trend(start_date, end_date, tower, track, period):
    ...
    try:
        ...
        q = f"""
            SELECT {bucket} Bucket,COUNT(*) Total,
                   SUM(CASE WHEN UPPER(ISNULL(tk.TicketType,''))='PARENT' THEN 1 ELSE 0 END) Parents,
                   SUM(CASE WHEN UPPER(ISNULL(tk.TicketType,''))='CHILD' THEN 1 ELSE 0 END) Children
            FROM qbr.Ticket tk {joins}
            WHERE tk.{date_col} IS NOT NULL
              AND {_date_filter('tk',date_col)} AND {scope}
            GROUP BY {group}
            ORDER BY Bucket
        """
        ...
        for row in rows:
            ...
            label_value = (
                bucket_value.strftime("%d %b %Y") if period == "day"
                else bucket_value.strftime("%d %b") if period == "week"
                else bucket_value.strftime("%b %Y") if period == "month"
                else f"Q{((bucket_value.month-1)//3)+1} {bucket_value.year}"
            )
            data.append({label: label_value, "Total": _safe_int(row.Total), "Parents": _safe_int(row.Parents), "Children": _safe_int(row.Children)})
...
def get_alert_frequency(start_date=None, end_date=None, tower=None, track=None):
    """Monitoring/device frequency from ticket rows whose Caller is EMS/CMSP."""
    db = SessionLocal()
    try:
        cols, joins, _tower, _track, scope = _ticket_context(db, "tk")
        date_col = "OpenedAt" if "OpenedAt" in cols else "CreatedAt"
        if "Caller" not in cols:
            return pd.DataFrame(columns=["Device", "AlertType", "Severity", "Count"])
        device_col = "ISNULL(tk.Device,'Unknown')" if "Device" in cols else ("ISNULL(tk.Part,'Unknown')" if "Part" in cols else "'Unknown'")
        q = f"""
            SELECT {device_col} Device,
                   'Monitoring-generated ticket' AlertType,
                   ISNULL(tk.Priority,'Unknown') Severity,
                   COUNT(*) AlertCount
            FROM qbr.Ticket tk {joins}
            WHERE UPPER(LTRIM(RTRIM(ISNULL(tk.Caller,'')))) IN ('EMS','CMSP')
              AND {_date_filter('tk',date_col)} AND {scope}
            GROUP BY {device_col},ISNULL(tk.Priority,'Unknown')
            ORDER BY AlertCount DESC,Device
        """
        rows = db.execute(text(q), _params(start_date, end_date, tower, track)).fetchall()
...
def get_parent_child_relation(start_date=None, end_date=None, tower=None, track=None):
    ...
    try:
        ...
        q = f"""
            SELECT b.ParentTicket,b.Tower,b.Track,COUNT(*) ChildCount,
                   MAX(b.Priority) Priority,MAX(b.State) State
            FROM (
                SELECT CAST(c.ParentTicketNumber AS nvarchar(255)) ParentTicket,
                       {tower_expr} Tower,{track_expr} Track,c.Priority,c.State
        ...
        parents = pd.DataFrame([
            {"ParentTicket": r.ParentTicket, "Tower": r.Tower, "Track": r.Track, "ChildCount": _safe_int(r.ChildCount), "Priority": r.Priority, "State": r.State}
            for r in rows
        ])
        ...
        children = pd.DataFrame([
            {"ChildTicket": r.ChildTicket, "ParentTicket": r.ParentTicket, "Tower": r.Tower, "Track": r.Track, "Priority": r.Priority, "State": r.State}
            for r in rows2
        ])
...
def get_volume_stats(start_date=None, end_date=None, tower=None, track=None):
    ...
    try:
        ...
        q = f"""
            ...
            SELECT MAX(DailyTotal) MaxCount,
                   MIN(DailyTotal) MinCount,
        ...
        if not row or row.MaxCount is None:
            return None
        return {
            "max_count": _safe_int(row.MaxCount),
            "min_count": _safe_int(row.MinCount),
            "avg_count": round(float(row.AvgCount), 1),
...
def get_tower_track_alerts(start_date=None, end_date=None, tower=None, track=None):
    """Severity summary of monitoring-generated ticket rows."""
    db = SessionLocal()
    try:
        cols, joins, tower_expr, track_expr, _scope = _ticket_context(db, "tk")
        date_col = "OpenedAt" if "OpenedAt" in cols else "CreatedAt"
        if "Caller" not in cols:
            return pd.DataFrame(columns=["Tower", "Track", "TotalAlerts", "Critical", "High", "Moderate"])
        q = f"""
            SELECT b.Tower,b.Track,COUNT(*) TotalAlerts,
                   SUM(CASE WHEN UPPER(ISNULL(b.Priority,'')) LIKE '%CRITICAL%' OR b.Priority='1' THEN 1 ELSE 0 END) Critical,
                   SUM(CASE WHEN UPPER(ISNULL(b.Priority,'')) LIKE '%HIGH%' OR b.Priority='2' THEN 1 ELSE 0 END) High,
                   SUM(CASE WHEN UPPER(ISNULL(b.Priority,'')) LIKE '%MODERATE%' OR UPPER(ISNULL(b.Priority,'')) LIKE '%MEDIUM%' OR b.Priority='3' THEN 1 ELSE 0 END) Moderate
            FROM (
                SELECT {tower_expr} Tower,{track_expr} Track,tk.Priority
                FROM qbr.Ticket tk {joins}
                WHERE UPPER(LTRIM(RTRIM(ISNULL(tk.Caller,'')))) IN ('EMS','CMSP')
                  AND {_date_filter('tk',date_col)}
            ) b
            WHERE (:tower IS NULL OR b.Tower=:tower)
              AND (:track IS NULL OR b.Track=:track)
            GROUP BY b.Tower,b.Track
        ...
        rows = db.execute(text(q), _params(start_date, end_date, tower, track)).fetchall()
        return pd.DataFrame([
            {"Tower": r.Tower, "Track": r.Track, "TotalAlerts": _safe_int(r.TotalAlerts), "Critical": _safe_int(r.Critical), "High": _safe_int(r.High), "Moderate": _safe_int(r.Moderate)}
            for r in rows
        ])
    finally:
        db.close()
