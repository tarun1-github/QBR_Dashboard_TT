...
def get_alert_total(start_date=None,end_date=None,tower=None,track=None):
    ...
    try:
        ...
        q=f"SELECT COUNT(*) FROM qbr.Ticket tk {joins} WHERE UPPER(LTRIM(RTRIM(ISNULL(tk.Caller,'')))) IN ('EMS','CMSP') AND {_date_filter('tk',date_col)} AND {scope}"
...
def get_alert_frequency(start_date=None,end_date=None,tower=None,track=None):
    ...
    try:
        ...
        q=f"SELECT {device} Device,'Monitoring-generated ticket' AlertType,ISNULL(tk.Priority,'Unknown') Severity,COUNT(*) AlertCount FROM qbr.Ticket tk {joins} WHERE UPPER(LTRIM(RTRIM(ISNULL(tk.Caller,'')))) IN ('EMS','CMSP') AND {_date_filter('tk',date_col)} AND {scope} GROUP BY {device},ISNULL(tk.Priority,'Unknown') ORDER BY AlertCount DESC,Device"
        rows=db.execute(text(q),_params(start_date,end_date,tower,track)).fetchall()
...
def get_tower_track_alerts(start_date=None,end_date=None,tower=None,track=None):
    ...
    try:
        ...
        q=f"SELECT b.Tower,b.Track,COUNT(*) TotalAlerts,SUM(CASE WHEN UPPER(ISNULL(b.Priority,'')) LIKE '%CRITICAL%' OR b.Priority='1' THEN 1 ELSE 0 END) Critical,SUM(CASE WHEN UPPER(ISNULL(b.Priority,'')) LIKE '%HIGH%' OR b.Priority='2' THEN 1 ELSE 0 END) High,SUM(CASE WHEN UPPER(ISNULL(b.Priority,'')) LIKE '%MODERATE%' OR UPPER(ISNULL(b.Priority,'')) LIKE '%MEDIUM%' OR b.Priority='3' THEN 1 ELSE 0 END) Moderate FROM (SELECT {tower_expr} Tower,{track_expr} Track,tk.Priority FROM qbr.Ticket tk {joins} WHERE UPPER(LTRIM(RTRIM(ISNULL(tk.Caller,'')))) IN ('EMS','CMSP') AND {_date_filter('tk',date_col)}) b WHERE (:tower IS NULL OR b.Tower=:tower) AND (:track IS NULL OR b.Track=:track) GROUP BY b.Tower,b.Track ORDER BY TotalAlerts DESC,b.Tower,b.Track"
        rows=db.execute(text(q),_params(start_date,end_date,tower,track)).fetchall()
