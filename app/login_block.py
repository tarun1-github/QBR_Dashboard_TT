...
Authentication is persisted in a browser cookie containing only a signed
username. A browser refresh therefore keeps the user signed in; only the
explicit Sign out action removes the cookie.
"""
...
def _cookie_manager():
    ...
    if "_qbr_cookie_manager" not in st.session_state:
        st.session_state["_qbr_cookie_manager"] = stx.CookieManager(key="qbr_auth_cookie_manager")
    return st.session_state["_qbr_cookie_manager"]


def _sign_username(username: str, issued: int) -> str:
    payload = f"{username}|{issued}"
    sig = hmac.new(COOKIE_SECRET.encode("utf-8"), payload.encode("utf-8"), hashlib.sha256).hexdigest()
...
def _save_session_to_cookie(user: dict):
    ...
    cookies.set(
        COOKIE_NAME,
        _sign_username(username, int(time.time())),
        key="qbr_auth_set",
        path="/",
...
def _read_session_cookie():
    ...
    if cookies is None:
        return None
    try:
        return _verify_cookie(cookies.get(COOKIE_NAME) or "")
    except Exception:
        return None
...
def _clear_cookie():
    ...
    if cookies is not None:
        try:
            cookies.delete(COOKIE_NAME, key="qbr_auth_delete")
        except Exception:
            pass
...
def success_message(title: str, detail: str = ""):
    st.markdown(
        f'<div class="qbr-toast success"><div class="qbr-toast-icon">✓</div><div><b>{title}</b><span>{detail}</span></div></div>',
...
def error_message(title: str, detail: str = ""):
    st.markdown(
        f'<div class="qbr-toast error"><div class="qbr-toast-icon">!</div><div><b>{title}</b><span>{detail}</span></div></div>',
        unsafe_allow_html=True,
    )
...
def info_message(title: str, detail: str = ""):
    st.markdown(
        f'<div class="qbr-toast info"><div class="qbr-toast-icon">i</div><div><b>{title}</b><span>{detail}</span></div></div>',
        unsafe_allow_html=True,
    )
...
def clear_session():
    """Explicit logout. Browser refresh does not call this function."""
    _clear_cookie()
...
def _auth_css():
    st.markdown(
        ...
        .qbr-auth-brand{padding:24px 22px;border-radius:26px;text-align:center;background:linear-gradient(135deg,#082c49,#106d88,#1da98c);color:#fff;box-shadow:10px 11px 0 rgba(8,44,73,.15),0 18px 35px rgba(8,44,73,.20);margin-bottom:18px}
        .qbr-auth-brand h1{margin:0;font:1000 30px 'Segoe UI',Aptos,sans-serif}.qbr-auth-brand p{margin:7px 0 0;font-size:12px}
        .qbr-auth-card{padding:22px 28px 28px;border-radius:28px;background:linear-gradient(145deg,#fff,#eaf6f8);border:1px solid #d2e5e9;box-shadow:14px 15px 35px rgba(11,52,74,.16),-8px -8px 20px rgba(255,255,255,.9)}
        ...
        div[data-testid="stTextInput"] input:focus{border-color:#1595a5!important;box-shadow:0 0 0 4px rgba(21,149,165,.12),4px 5px 0 rgba(15,39,66,.09)!important}
        ...
        .qbr-auth-note,.qbr-password-rule{width:390px;max-width:100%;margin:10px auto;padding:11px 14px;border-radius:14px;font-size:12px;box-shadow:3px 4px 0 rgba(15,39,66,.07)}
        .qbr-auth-note{background:#eaf7fb;border:1px solid #add9e3;color:#28596b}.qbr-password-rule{background:#fff7df;border:1px solid #ead28a;color:#725a00}
        .qbr-toast{display:flex;gap:11px;align-items:center;padding:12px 15px;border-radius:15px;margin:11px 0;box-shadow:4px 5px 0 rgba(15,39,66,.09);font-size:13px}.qbr-toast span{display:block;font-weight:600;margin-top:3px}.qbr-toast-icon{width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:1000;color:#fff}.qbr-toast.success{background:#e7faef;border:1px solid #68c98d;color:#12623a}.qbr-toast.success .qbr-toast-icon{background:#1d9b5b}.qbr-toast.error{background:#fff0f0;border:1px solid #df8a8a;color:#982323}.qbr-toast.error .qbr-toast-icon{background:#d43a3a}.qbr-toast.info{background:#eaf5ff;border:1px solid #7fb8e8;color:#185486}.qbr-toast.info .qbr-toast-icon{background:#237db7}
...
def render_login():
    ...
    with center:
        ...
        if login_clicked:
            ...
            if not alias:
                error_message("Username required.", "Please enter username.")
                return
            db = SessionLocal()
            try:
                account = get_user(db, alias)
